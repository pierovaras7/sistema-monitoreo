<div class="custom-px">
    ...
    <div x-data="{ modalAula: false, showNotification: false, timeout: null }" 
        ...
        <div class="flex flex-col sm:flex-row sm:space-x-4">
            ...
            <button 
                @click="$wire.call('abrirModalAgregar').then(() => modalAula = true)"
            ...
            <div class="flex items-center sm:w-5/6">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" id="simple-search" wire:model.live="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mx-2" placeholder="Buscar Aula" required />
                </div>
            </div>

        </div>
        <!-- Modal -->
        <div x-show="modalAula" 
            x-transition 
            class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center z-50"
            style="display: none;"
            @keydown.escape.window="modalAula = false"
            @click.self="modalAula = false"   
            >
            <div class="relative p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div wire:key="aula-form-{{ $modoEdicion ? 'edit' : 'add' }}" class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            {{ $modoEdicion ? 'Actualizar aula' : 'Agregar aula' }}                        
                        </h3>
                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" @click="modalAula = false">
                    ...
                    <form class="p-4 md:p-5" wire:submit.prevent="guardar" x-data="{ showError: false }">
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div 
                                x-data="programaSelect()" 
                                class="relative col-span-2"
                            >
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Seleccionar Programa</label>
                                
                                <button @click="open = !open" type="button" class="w-full text-left bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg p-2.5">
                                    <span x-text="selectedName"></span>
                                </button>

                                <div x-show="open" @click.away="open = false" class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg">
                                    <input type="text" x-model="search" placeholder="Buscar programa..." class="w-full p-2 text-xs border-b dark:bg-gray-700 dark:text-white">
                                    <ul class="max-h-60 overflow-y-auto">
                                        <template x-for="p in filtered" :key="p.id">
                                            <li @click="select(p.id)" class="px-4 py-2 cursor-pointer hover:bg-blue-100 dark:hover:bg-gray-600">
                            ...
                            <div class="col-span-2">
                                ...
                                <input wire:model.live="codigo" type="codigo" name="codigo" id="codigo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Ingrese el codigo del aula.">
                            ...
                            <div 
                                ...
                                class="relative col-span-2"
                            >
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Asignar Asesor</label>
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg">
                                    <input type="text" x-model="search" placeholder="Buscar asesor..." class="w-full p-2 text-xs border-b dark:bg-gray-700 dark:text-white">
                                    <ul class="max-h-60 overflow-y-auto">
                                        <template x-for="a in filtered" :key="a.id">
                                            <li @click="select(a.id, a.nombre)" class="px-4 py-2 cursor-pointer hover:bg-blue-100 dark:hover:bg-gray-600">
    ...
    function programaSelect() {
        return {
            ...
            select(id) {
                this.selectedId = id;
                this.open = false;
                Livewire.dispatch('input', { name: 'programas_id', value: id });
            },
            get selectedName() {
